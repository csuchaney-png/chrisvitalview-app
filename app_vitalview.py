...
st.markdown(
    ...
    <style>
      .stApp {
        background: radial-gradient(circle at top left, #f5fbff 0, #ffffff 45%, #f9f9ff 100%);
      }
      ...
      .stTabs [data-baseweb="tab"][aria-selected="true"] {
        border-bottom: 3px solid #0A74DA !important;
...
def to_excel_bytes(df: pd.DataFrame) -> bytes:
    """Export a DataFrame as an Excel file in memory."""
    buffer = BytesIO()
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        df.to_excel(writer, index=False, sheet_name="VitalView")
...
if not priority_df.empty:
    ...
    if FEATURES.get("exports", False):
        ...
        st.download_button(
            ...
            data=to_excel_bytes(priority_df),
            file_name="priority_list.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="priority_xlsx_dl"
        )
else:
